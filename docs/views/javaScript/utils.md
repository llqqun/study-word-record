...
```js
//rules = [{ key: '', required: true, message: '必填项', validator: (rule, value, callback) => {} }]
function verifyObjectForm(target, rules) {
      let errors = [];
      if (target && Object.prototype.toString.call(target).indexOf('Object') > -1) {
        for (let field in rules) {
          if (!rules.hasOwnProperty(field)) continue;

          let fieldRules = rules[field];

          // 如果数据中不存在该字段且该字段的第一条规则要求必填，则添加错误信息
          if (!target.hasOwnProperty(field) && fieldRules[0].required) {
            errors.push({ field: field, message: fieldRules[0].message });
            continue;
          }
...
const fileCach = new File([blobData], '测试文件.docx', {
...
通过设置 DataSet API 设置临时存放 sr 路径的 data-src 属性

当图片出现在视窗位置,则修改 src 为 data-src
...
input 标签声明 type 为 file 即可进行文件上传操作
...
1.在 Vue 中实现首屏动画,直接修改 index.html
可以先在 index 页面固定一个动画效果,当页面加载完成后再取消这个动画
...
@keyframes before {
  0% {
    width: 20px;
    box-shadow: 20px -20px rgba(225, 20, 98, 0.75), -20px 20px rgba(111, 202, 220, 0.75);
  }
  35% {
    width: 100px;
    box-shadow: 0 -20px rgba(225, 20, 98, 0.75), 0 20px rgba(111, 202, 220, 0.75);
  }
  70% {
    width: 20px;
    box-shadow: -20px -20px rgba(225, 20, 98, 0.75), 20px 20px rgba(111, 202, 220, 0.75);
...
@keyframes after {
  0% {
    height: 20px;
    box-shadow: 20px 20px rgba(61, 184, 143, 0.75), -20px -20px rgba(233, 169, 32, 0.75);
  }
  35% {
    height: 100px;
    box-shadow: 20px 0 rgba(61, 184, 143, 0.75), -20px 0 rgba(233, 169, 32, 0.75);
  }
  70% {
    height: 20px;
    box-shadow: 20px -20px rgba(61, 184, 143, 0.75), -20px 20px rgba(233, 169, 32, 0.75);
